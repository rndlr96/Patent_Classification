...
import logzero
logzero.setup_default_logger(logfile='./laha.log')
...
def main(data_cnf, model_cnf, mode, tree_id):
    ...
    if mode is None or mode == 'train':
        ...
        test_x, test_labels = get_data(data_cnf['test']['texts'], data_cnf['test']['labels'])
        
        if 'size' in data_cnf['valid']:
            valid_x = test_x[:data_cnf['valid']['size']]
            valid_labels = test_labels[:data_cnf['valid']['size']]
        else:
        ...
        if 'cluster' not in model_cnf:
            ...
            model = Model(network=AttentionRNN, labels_num=labels_num, model_path=model_path, emb_init=emb_init,  **data_cnf['model'], **model_cnf['model'])
